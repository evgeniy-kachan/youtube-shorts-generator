...
2. ЗАГОЛОВОК YouTube (title) — безопасный для алгоритмов:
   ...
   - Включай КОНКРЕТИКУ из видео: имена, числа, факты
   - Если уместно — цитата спикера от первого лица
   - Формат: вопрос, утверждение-провокация или факт с числом
   - НЕ спойлерить главную мысль
   - ИЗБЕГАЙ: политика, госдолг, военные, заговоры, кризис
   - Примеры хороших заголовков:
     «Маск: страны скоро станут анахронизмом»
     «3 года до дефляции — вот почему»
     «Что произойдёт, когда ИИ замкнёт цикл»
     «Почему 90% людей неправильно ставят цели»

3. АЛЬТЕРНАТИВНЫЕ ЗАГОЛОВКИ (title_alternatives) — 3 варианта разных стилей:
   - Числовой (с конкретным числом/фактом из видео)
   - Цитатный (прямая цитата спикера, от первого лица)
   - Интригующий (вопрос или провокация)
   Все должны быть безопасными для YouTube.

4. ЗАГОЛОВОК TikTok (title_tiktok):
   - Может быть провокационнее, острее, эмоциональнее
   - Политика, госдолг, военные — допустимы
   - Пример: «$35 трлн долга — и один способ его решить»

5. ДВА ОПИСАНИЯ:
   а) YouTube (description) — безопасное:
      - 2-4 коротких строки, каждая с новой строки
      - Суть видео через технологии/науку/будущее
...
{{
  "category": "технологии",
  "title": "Заголовок для YouTube (безопасный)",
  "title_alternatives": [
    "Числовой вариант",
    "Цитатный вариант",
    "Интригующий вариант"
  ],
  "title_tiktok": "Заголовок для TikTok (может быть острее)",
  "description": "Описание для YouTube\\nНесколько строк",
  "description_tiktok": "Описание для TikTok\\nМожет быть острее",{guest_json_field}
  "hashtags": ["#тема", "#AI", "#будущее"]
}}"""
