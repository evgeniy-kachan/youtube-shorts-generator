...
class DeepSeekClient:
    ...
    def generate_shorts_description(
        ...
        # --- Build guest-related prompt parts ---
        guest_context = ""
        guest_json_field = ""
        guest_rule = ""
        if guest_name:
            ...
            guest_rule = (
                f"\n6. О ГОСТЕ (guest_bio):"
                f"\n   - 1-3 предложения: кто такой {guest_name}, чем известен"
                f"\n   - Только проверенные факты. Если не знаешь — пустая строка \"\""
            )

        prompt = f"""ТЕКСТ ВИДЕО (используй ТОЛЬКО эти данные):
Английский: {text_en[:1500]}
Русский: {text_ru[:1500]}
Длительность: {duration:.0f} сек{guest_context}

Создай описание на РУССКОМ. Два варианта: YouTube (безопасный) и TikTok (острый).

YouTube НЕ продвигает: политику, госдолг, военные, теории заговоров.
Для YouTube — формулируй через науку, технологии, будущее.
TikTok — без ограничений.

ПРАВИЛА:

1. КАТЕГОРИЯ (category) — одно слово строчными:
   саморазвитие, психология, мышление, продуктивность, мотивация,
   бизнес, финансы, карьера, лидерство, отношения, здоровье,
   технологии, наука, философия, общество

2. ЗАГОЛОВОК YouTube (title):
   - 5-10 слов
   - Используй ДОСЛОВНЫЕ цитаты спикера в кавычках — не пересказывай
   - Бери конкретные детали ИЗ ТЕКСТА: имена, числа, факты
   - Заголовок должен подходить ТОЛЬКО к этому видео
   - Для YouTube: без политики, госдолга, военных

   ЗАПРЕЩЕНО в заголовках:
   - Клише: «вот почему», «изменит всё», «секрет», «правда о», «никто не говорит», «вот что будет»
   - Абстракции: «через призму», «на тему», «разговор о»
   - Обращения к зрителю: «ты точно», «готов поспорить», «вот доказательство»
   - Всё, что звучит как написано AI

3. АЛЬТЕРНАТИВЫ (title_alternatives) — 3 штуки:
   - Вопрос (конкретный, с деталью из текста)
   - Цитата (дословная фраза спикера в кавычках)
   - Противопоставление (X vs Y, A — но B)

4. ЗАГОЛОВОК TikTok (title_tiktok):
   - Острее, эмоциональнее, можно политику и $-суммы
   - Тоже с конкретикой из текста, не общие фразы

5. ОПИСАНИЯ:
   YouTube (description):
   - 2-3 строки, каждая с новой строки
   - Конкретные факты/цитаты из видео, не общие фразы
   - 1 эмодзи максимум
   - НЕ писать: «смотри до конца», «досмотри», «не пропусти»

   TikTok (description_tiktok):
   - 1-3 строки, дерзкий тон
   - Можно политику, экономику
   - Конкретика из текста, ничего не выдумывай
{guest_rule}
ГЛАВНОЕ ПРАВИЛО: каждый факт, цитата, число — ТОЛЬКО из предоставленного текста. Ничего не придумывай.
...
{{
  ...
  "hashtags": ["#...", "#...", "#...", "#...", "#..."]
        ...
        messages = [
            {"role": "system", "content": "Копирайтер для коротких видео. Пишешь живым языком, берёшь цитаты и факты из текста. Без клише, без выдумок. JSON."},
