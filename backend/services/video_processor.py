...
class VideoProcessor:
    ...
    def _generate_dialogue_subtitles(
        ...
        for turn in dialogue:
            ...
            # Diagnostic check: warn if no chunks were generated (should not happen with fixed fallback)
            if chunks_added == 0:
                logger.error(
                    "CRITICAL: No subtitle chunks generated for speaker %s (%.2f–%.2fs) despite fallback logic. "
                    "This indicates a bug in subtitle generation.",
                    speaker_id,
                    relative_start,
                    relative_end,
                )
